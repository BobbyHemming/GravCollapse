"""A FILE holding all the variables, constants and parameters required to describe a system of particles in a
   gravitational system. It will also be able generate """

import numpy as np
import random
from ConstantsVariables import Variables
from SolarSystemInfo import Orbitconditions
...
def randomcluster(variables, mass):
    ...
    while count < variables[0]:
        ...
        mass[count-1] = random.uniform(4, 16)*(10 ** 30)

    if variables[8] == 2:
        kinematics[0, 0, :] = [0, 0, 0]
        kinematics[0, 1, :] = [0, 0, 0]
        kinematics[0, 2, :] = [0, 0, 0]
        mass[0] = random.uniform(40, 40)*(10 ** 30)

        kinematics[1, 0, :] = [0.3*Variables[6], 0.3*Variables[6], 0.3*Variables[6]]
        kinematics[1, 1, :] = [0, 0, 0]
        kinematics[1, 2, :] = [0, 0, 0]
        mass[1] = random.uniform(40, 40) * (10 ** 30)

        kinematics[2, 0, :] = [-0.3*Variables[6], 0.3*Variables[6], -0.3*Variables[6]]
        kinematics[2, 1, :] = [0, 0, 0]
        kinematics[2, 2, :] = [0, 0, 0]
        mass[2] = random.uniform(40, 40) * (10 ** 30)

        kinematics[3, 0, :] = [-0.3*Variables[6], -0.3*Variables[6], 0.3*Variables[6]]
        kinematics[3, 1, :] = [0, 0, 0]
        kinematics[3, 2, :] = [0, 0, 0]
        mass[3] = random.uniform(40, 40) * (10 ** 30)
